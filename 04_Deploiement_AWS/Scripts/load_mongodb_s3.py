...
# Charge .env si present (dev local)
load_dotenv()
...
# ============================================================
# CONFIG (via variables d'environnement uniquement)
# ============================================================
...
# ============================================================
# SCHEMA VALIDATOR (aligne sur la version locale : strict/error)
# ============================================================
SCHEMA = {
    "$jsonSchema": {
        "bsonType": "object",
...
def redact_uri(uri: str) -> str:
    """Masque le mot de passe dans l'URI pour les logs."""
...
def validate_quality(coll):
    ...
    by_source = {
        r["_id"]: r["count"]
        for r in coll.aggregate([{"$group": {"_id": "$source", "count": {"$sum": 1}}}])
    }
...
def main():
    ...
    try:
        ...
        logger.info(
            f"Periode: {quality['date_range']['min']} -> {quality['date_range']['max']}"
        )
