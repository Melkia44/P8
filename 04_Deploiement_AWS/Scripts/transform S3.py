...
def extract_airbyte(rec):
    """Extrait donnees du wrapper Airbyte."""
...
def wind2deg(txt):
    """Direction cardinale vers degres (ex: 'NNE' -> 22.5)."""
    return WIND.get(txt.strip(), np.nan) if txt and isinstance(txt, str) else np.nan


def sanitize(obj):
    """Nettoyage recursif : NaN/Inf -> None, numpy -> python natif."""
    if obj is None:
        return None
...
def detect_source(recs):
    """Detecte le type de source par inspection des champs."""
    if not recs:
        return "unknown"
...
def parse_infoclimat(recs):
    """Parse les records InfoClimat vers le schema unifie."""
...
def parse_wu(recs, sid, s3_path):
    """Parse les records Weather Underground vers le schema unifie."""
    logger.info(f"Parse WU ({sid})")
    meta = WU_META.get(
        sid,
        {
            "station_name": f"Unknown_{sid}",
            "latitude": None,
            "longitude": None,
            "elevation": None,
            "station_type": "weather_underground",
...
def infer_station(path):
    """Infere l'identifiant station depuis le chemin S3."""
...
# ============================================================
def validate(df):
    """Genere un rapport de qualite sur le DataFrame."""
    total = len(df)
    if total == 0:
    ...
    m = {
        ...
        "null_rates": {
            c: round(df[c].isna().sum() / total * 100, 2) for c in COLS
        },
        "duplicates": int(
...
def main():
    ...
    try:
        ...
        # DEDUPLICATION sur (station_id, timestamp)
